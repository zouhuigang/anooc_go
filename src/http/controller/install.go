...
func (self InstallController) SetupConfig(ctx echo.Context) error {
	...
	// config/env.ini 存在
	if db.MasterDB != nil {
		if model.DefaultInstall.IsTableExist(ctx) {
...
func (self InstallController) DoInstall(ctx echo.Context) error {
	...
	if model.DefaultInstall.IsTableExist(ctx) {
		if model.DefaultInstall.HadRootUser(ctx) {
			return ctx.Redirect(http.StatusSeeOther, "/")
		}
	}
	...
	if step == 2 {
		...
		err := model.DefaultInstall.CreateTable(ctx)
		if err != nil {
			data["err"] = "创建数据表失败！"
			return renderInstall(ctx, "install/install.html", data)
		}

		err = model.DefaultInstall.InitTable(ctx)
		if err != nil {
			data["err"] = "初始化数据表失败！"
			return renderInstall(ctx, "install/install.html", data)
		}
